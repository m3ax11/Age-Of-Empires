...
Nach einem weiteren Screenshot wurde klargestellt, dass nicht mehr die Terrain-Textur, sondern die politischen Laenderfarben selbst zu verwaschen wirkten. Deshalb setzt `common/defines/zz_AOEIW_graphics.lua` nun ausschliesslich `COUNTRY_COLOR_SATURATION_MODIFIER = 0.90` und `COUNTRY_COLOR_BRIGHTNESS_MODIFIER = 0.88` gegenueber Vanilla `0.60/0.80`. Das erzeugt deutlich kraeftigere, klarere Flaechenfarben, ohne Beleuchtung, Texturen oder Grenzstaerke erneut zu veraendern.
