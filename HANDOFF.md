...
The corrected installed build was then launched with only `mod/Age_Of_Empires_RT56_Edition.mod` enabled. `setup.log` proves AOE-owned files loaded, `game.log` reached the 1936 history pass after loading 4,469 defines and 13,535 provinces, and the HOI4 process remained responsive. The fresh error log contains no match for AOE, the roster system or the six changed Pacific state files. The apparent active map-mode count of one is only Paradox's fully commented `common/map_modes/documentation.txt`; both actual scripted modes remain disabled. The running native HOI4 window was not exposed to the available app-capture surface, so the brighter political-map result still needs a manual visual A/B check. If it remains too textured after this shader/define correction, inspect `map/terrain/mud_diffuse_rgb_gloss_a_0.dds` first and `atlas0.dds` second; do not replace RT56 geometry assets wholesale.

The user's first visual check still found the political map too painted, so the targeted second pass removed both `map/terrain/mud_diffuse_rgb_gloss_a_0.dds` and `map/terrain/atlas0.dds`. The game now inherits the current vanilla ground texture and terrain atlas. World/water colormaps, normal map, terrain geometry, province geometry and state borders remain untouched. This is the end of the previously identified presentation overrides; further visual adjustment should be based on a new screenshot rather than broad texture replacement.

That exact second-pass build was synchronized and launched with only the local AOE descriptor enabled. The engine loaded 4,469 defines and 13,535 provinces, reset the game and executed history through `1936.1.1.12`; the process remained responsive. The fresh error log returned zero targeted matches for AOE, map-roster, the removed shader/terrain assets or the six changed Pacific states.

The next screenshot clarified that the remaining complaint concerned washed-out political country colours rather than terrain texture. `common/defines/zz_AOEIW_graphics.lua` now raises only country-colour saturation/brightness to `0.90/0.88` from vanilla `0.60/0.80`. Lighting, texture selection and border presentation remain unchanged.

### Imperial roster expansion (15 September 2026)

The user authorized every imperial and regional playable-country proposal from the preceding design discussion. `IMPERIAL_ROSTER_EXPANSION.md` is the authoritative state/overlord record. Nineteen existing tags now own compact 1936 territories as Japanese, British, French, Belgian, Italian, Spanish, Persian, Portuguese or German subjects; five Chinese regional governments now survive the opening Qing crisis; and new tags `IAF`, `MAF` and `GPA` provide Italian East Africa plus dynamic German Mittelafrika/Pacific administrations. Every new start actor receives a distinct identity spirit, player introduction and connected autonomy or regional-settlement decisions. Existing MAL/EST/VIN/MLI/KAZ/GEO subjects join the autonomy system. German East Asia keeps its valid Qingdao OOB location but no longer recruits Malaysia's inherited character roster. The Ottoman start remains intact.

The administration content is now complete at source level. `common/national_focus/AOEIW39_imperial_administrations_focus.txt` supplies dedicated fourteen-focus trees to IAF, MAF and GPA with political, economic and military choices that retain distinct permanent outcomes after reconvergence. Their English and German text was individually authored around Massaua/Mogadischu, Duala/Congo and Rabaul/Pacific institutions. All choice spirits and the three country-specific consolidation outcomes are localized. Default and ideology flags exist at all three required sizes, and `common/countries/colors.txt` registers their UI colours. The static validator parsed 5,295 script files without a proven structural, AOE-reference, localisation, map or scenario-contract finding. An exact installed-build runtime gate remains required after synchronization.

The first exact-build launch reached the 1936 history pass and exposed three AOE-owned engine-schema issues: two country ideas used the unit-stat-only `naval_range` key, the imperial budget AI condition used obsolete `stability` trigger syntax, and IAF attempted to recruit a character already assigned to ERI. These are corrected with `navy_max_range_factor`, `has_stability`, and a dedicated historical colonial governor for IAF. The user then asked that Age of Empires not be launched again while an unrelated HOI4 RP session runs. Static validation passes after the corrections; the clean second runtime gate is therefore deliberately outstanding.

The user's next manual AOE launch produced a reproducible access-violation crash after the 1936 history pass. All three crash logs ended immediately after `interface/frontendmainview.gui` failed to find the now-required `subscription_size` control in `mainmenu_panel_bottom`. That file was an inherited old RT56 frontend override with Paradox's subscription controls commented out. The override has been removed so HOI4 1.19 supplies its current frontend GUI; the separate AOE frontend background, logo/loading assets and thumbnail remain active. This fix was synchronized without starting the game, in accordance with the user's standing request.

A subsequent manual launch removed the `subscription_size` error and reached the corrected main menu, but still crashed exactly as `frontend.cpp` completed startup. The remaining non-vanilla frontend subsystem was the selectable ten-entry AOE background database and its custom preview-sprite registry. Both registries are removed. The AOE title image remains active through the ordinary `GFX_frontend_bg` and `GFX_frontend_bg_basic` overrides, avoiding the additional background-manager path. This requires another user-run runtime check because the user asked Codex not to launch the mod.

That narrower change did not resolve the access violation: the next dump again ended at the same executable address immediately after `frontend.cpp` completed. All remaining inherited frontend overrides are therefore removed: the custom main/basic background sprite definitions, the old game-setup GUI, RT56 preview sprites/background database and the legacy non-mipmapped `logo_game.dds`. HOI4 now owns the entire frontend path with its current 1.19 files. The AOE source/title DDS files, thumbnail and box art remain in the repository for a later version-native reintroduction, but no active frontend definition references them. This deliberately prioritizes a stable start over custom menu presentation.

Future Git commits use concrete categorized subjects: `Fix:`, `Content:`, `Focus:`, `Map:`, `Localization:` or `Docs:`, followed by the affected country or system and the actual result.
