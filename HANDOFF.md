...
`MAP_VISUAL_BLOCKER_REPORT.md` proves that the textured screenshot is the normal political map, not either disabled scripted map mode. The actual dark presentation comes from the hard-coded `return 0.42f` permanent dusk in `gfx/FX/standardfuncsgfx.fxh`, reinforced by `common/defines/zz_AOEIW_graphics.lua` brightness/saturation/border overrides and a substantially different mud texture. No rendering edit has yet been made because the user required this report before implementation. The Arab roster is already graph-connected (SAU seven states in one component; OMA four states in one component), so no Arab state transfer is recommended. The report also records the requested future Pacific transfers: 634/1096 to AST as non-core administrations and 1136/1139/723/1140 to NZL as cores.
