...
The major-power standing rule is now explicit: theatres remain compatible by default unless a specific political or logistical conflict makes simultaneous pursuit implausible. Russia's divergence-aware structural report remains unapproved and its implementation is deferred at the user's request. The USA pass is implemented: Atlantic/Pacific methods now diverge, the two theatres remain compatible and jointly feed the Two-Ocean capstone, Field Workshops/Signals are exclusive, TVA and the Arsenal are connected to their capstones, Boston Congress outcomes differ, and Pacific reunion now removes the secession identity. Static validation and the graph audit are clean. The active queue is now United Kingdom; its report is complete in `FOCUS_STRUCTURE_REWORK.md`, but **no British implementation is authorized yet**. Per-country approval remains mandatory.

The cross-country divergence rule is now mandatory and its central ledger is `FOCUS_REWARD_PROFILE_REGISTER.md`. The British proposal was revised before implementation: Admiralty detection/coordination, Imperial Preference and subject control, BEF reinforcement/defence, and Ministry of Economic Warfare intelligence/arms output replace the earlier profiles that duplicated France and the USA. The ledger also records ten implemented-country conflict pairs across Denmark, Norway, Ireland, France and the USA, with narrow proposed corrections. No retrospective correction is authorized. Ask for approval one conflict pair at a time; do not batch their implementation. Russia's unapproved proposal also conflicts with France in all four proposed profiles and must be rewritten before it can return to the active queue.
...
Pair 6 was separately approved for closure with no additional gameplay edit. The approved pair 5 profile reduced the France Mediterranean Military versus USA Atlantic Mission overlap from 2/3 to 0/3, so another modifier change would have been redundant. Pair 7 is the next approval gate. Its full-register proposal uses sortie efficiency, air-mission efficiency and air-base construction for the USA Pacific mission; it shares at most one category with every current registered maritime profile and makes no gameplay change before approval.

The user then replaced the remaining individual gates with one consolidated register-closure proposal. The complete proposal is in `FOCUS_REWARD_PROFILE_REGISTER.md` and includes the still-open pairs 1–4 found in the repository as well as Russia and pairs 7–10; omitting pairs 1–4 would make a conflict-free claim false. Pair 3 is a cascade from the Irish pair-2 correction, and pair 10 is a cascade from the USA pair-9 correction. Russia's four method profiles were rewritten around broad-gauge mobilisation, winter movement, the Black Sea export corridor and Sevastopol mine warfare; its industrial endgame was also checked against the USA Arsenal. The proposed final matrix has zero 2/3 conflicts. No gameplay file has been changed for this consolidated proposal; one explicit approval is required before implementing the whole batch.
