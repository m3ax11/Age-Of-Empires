...
The major-power standing rule is now explicit: theatres remain compatible by default unless a specific political or logistical conflict makes simultaneous pursuit implausible. Russia's divergence-aware structural report remains unapproved and its implementation is deferred at the user's request. The active queue is now USA followed by United Kingdom. The USA report is complete in `FOCUS_STRUCTURE_REWORK.md`, but **no USA implementation is authorized yet**. Per-country approval remains mandatory.
